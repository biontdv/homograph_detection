...
# Diubah ke dataset yang baru
DATASET_PATH = Path("./Cleaned_7000_Malicious_Homographs.csv") 
TIMEOUT = 15  # seconds
...
with DATASET_PATH.open("r", encoding="utf-8", newline="") as f:
    ...
    for r in reader:
        # Menyesuaikan pengecekan kolom ke dataset baru
        if 'url' not in r or 'attack_type' not in r:
            raise SystemExit("CSV must contain 'url' and 'attack_type' columns")
        rows.append(r)

total_domains = len(rows)

# init results structure per case (menggunakan attack_type sebagai case)
for r in rows:
    case = r['attack_type']
...
print(f"✨ Memulai pengujian terhadap {total_domains} domain malicious (Homograph)...\n")
...
for idx, row in enumerate(rows, start=1):
    ...
    case = row['attack_type']
    
    # Jika url di dataset belum memiliki http://, kita tambahkan
    ...
    progress = f"Scanning {scanned}/{total_domains} — {domain[:30]}... ({case})"
...
# print per-case
print("================== HASIL PENGUJIAN PER ATTACK TYPE ==================")
...
print(f"⏱️  Avg. Duration (ms)    : {avg_duration_total:.2f}")   
print(f"💾 Avg. Memory (MB)      : {avg_memory_total:.4f}")      
print("===============================================================\n")
...
try:
    with OUTPUT_SUMMARY.open('w', encoding='utf-8') as out:
        out.write(f"Pengecekan {total_domains} domain homograph\n\n")
        out.write("PER ATTACK TYPE:\n")
        for case, r in results.items():
            total = r['detected'] + r['missed'] + r['error']
            accuracy = (r['detected'] / total * 100) if total > 0 else 0.0
            avg_dur = statistics.mean(r['durations']) if r['durations'] else 0.0
